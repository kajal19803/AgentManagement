...
// const { execFile } = require('child_process'); // Virus scan disabled
...
exports.uploadAndDistribute = async (req, res) => {
  try {
    ...
    data.forEach((entry, i) => chunks[i % agents.length].push(entry));
    const uploadedAt = new Date();

    await Promise.all(chunks.map((chunk, i) => DistributedList.create({ agent: agents[i]._id, list: chunk, uploadedAt })));
...
exports.getLatestList = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1, limit = parseInt(req.query.limit) || 5, search = req.query.search || '', skip = (page - 1) * limit;
    const latest = await DistributedList.findOne().sort({ uploadedAt: -1 }).select('uploadedAt');
    if (!latest) return res.json({ lists: [], total: 0 });

    const latestDate = latest.uploadedAt, match = { uploadedAt: latestDate };
    if (search) match.$or = [{ 'agent.name': { $regex: search, $options: 'i' } }, { 'agent.email': { $regex: search, $options: 'i' } }];

    const lists = await DistributedList.aggregate([
      { $match: { uploadedAt: latestDate } },
      { $lookup: { from: 'users', localField: 'agent', foreignField: '_id', as: 'agent' } },
      { $unwind: '$agent' },
      { $match: search ? match : { uploadedAt: latestDate } },
      { $sort: { 'agent.name': 1 } },
      { $skip: skip },
      { $limit: limit }
    ]);
...
exports.getHistory = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1, limit = parseInt(req.query.limit) || 5, search = req.query.search || '';
    const min = parseInt(req.query.minAgents) || 0, max = parseInt(req.query.maxAgents) || Infinity, skip = (page - 1) * limit;
    ...
    })).filter(g => g.uploadedAt && g.uploadedAt.includes(search) && g.agentsCount >= min && g.agentsCount <= max);
...
exports.getHistoryByTimestamp = async (req, res) => {
  try {
    const timestamp = new Date(req.params.timestamp), page = parseInt(req.query.page) || 1, limit = parseInt(req.query.limit) || 10;
    const search = req.query.search || '', skip = (page - 1) * limit;
    const matchStage = { uploadedAt: { $eq: timestamp } };
    const match = search ? {
      ...matchStage,
      $or: [
    ...
    const lists = await DistributedList.aggregate([
      { $match: matchStage },{ $lookup: { from: 'users', localField: 'agent', foreignField: '_id', as: 'agent' } },
      { $unwind: '$agent' },{ $match: match },{ $sort: { 'agent.name': 1 } },{ $skip: skip },{ $limit: limit },
      {$project: {
          _id: 1, list: 1, uploadedAt: 1,
          'agent._id': 1, 'agent.name': 1, 'agent.email': 1, 'agent.deleted': 1
        }}
    ]);
     const countArr = await DistributedList.aggregate([
      { $match: matchStage },
      { $lookup: { from: 'users', localField: 'agent', foreignField: '_id', as: 'agent' } },
      { $unwind: '$agent' },{ $match: match },{ $count: 'count' }
    ]);
